...
function buildGradientVars(colors: string[]) {
  return GRADIENT_KEYS.reduce<Record<string, string>>((vars, key, index) => {
    const color = colors[Math.min(COLOR_MAP[index], colors.length - 1)] ?? colors[0] ?? "#F7D133";
    vars[key] = `radial-gradient(at ${GRADIENT_POSITIONS[index]}, ${color} 0px, transparent 50%)`;
    return vars;
  }, {
    "--gradient-base": `linear-gradient(${colors[0] ?? "#F7D133"} 0 100%)`,
  });
}
...
export default function BorderGlow({
  ...
  useEffect(() => {
    ...
    const stopAngleRise = animateValue({
      ease: easeInCubic,
      duration: 1500,
      end: 50,
      onUpdate: (value) => {
    ...
    const stopAngleFall = animateValue({
      ease: easeOutCubic,
      delay: 1500,
      duration: 2250,
      start: 50,
